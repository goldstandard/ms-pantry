...
Každý stav má barvu (`STATUS_STYLES`). Řazení `compareByExpiry` dává nejbližší nahoru,
bez data nakonec.
...
- **podle kategorií** (výchozí): sekce s nadpisem, v hlavičce agregace (Σ balení, Σ porcí),
  uvnitř řazeno podle expirace;
- **podle expirace** (plochý seznam napříč kategoriemi).
